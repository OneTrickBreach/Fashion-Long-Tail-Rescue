...
\textbf{Contrastive learning for recommendations.}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{metrics/EDA_Long_tail_phase1.png}
...
Its architecture is shown in \Cref{fig:arch_villain}.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{presentation/arch_villain.png}
    \caption{Villain architecture: SASRec with learnable popularity bias vector.}
...
Its architecture is shown in \Cref{fig:arch_hero}.

\textbf{Visual projection.}
...
\textbf{Contrastive learning head (InfoNCE).}
...
The InfoNCE loss is:
\begin{equation}
    \mathcal{L}_{\text{InfoNCE}} = -\log \frac{\exp(\text{sim}(\mathbf{a}, \mathbf{p}^+)/\tau)}{\exp(\text{sim}(\mathbf{a}, \mathbf{p}^+)/\tau) + \sum_{j}\exp(\text{sim}(\mathbf{a}, \mathbf{n}_j)/\tau)}
\end{equation}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{presentation/arch_hero.png}
    \caption{Hero architecture: BST with ResNet50 visual fusion and InfoNCE contrastive head.}
    \label{fig:arch_hero}
\end{figure}
...
The architecture for this loss extension is shown in \Cref{fig:arch_discovery}.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{presentation/arch_discovery_loss.png}
    \caption{Discovery loss extension: differentiable penalty on softmax mass over popular items, integrated into the multi-objective loss.}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{presentation/model_comparison.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{presentation/training_curves.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{pareto/pareto_front.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{pareto/tail_rate_curve.png}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{presentation/ablation_visual_lift.png}
...
Results are shown in \Cref{fig:cold_start}.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{presentation/cold_start_comparison.png}
...
\textbf{Hardware.} All experiments run on a single NVIDIA RTX 5070 Ti (12~GB VRAM) with an Intel/AMD desktop CPU and 32~GB RAM.

\textbf{Software.} PyTorch 2.10 (CUDA 12.8), Python 3.11, NumPy, Pandas, scikit-learn, timm (ResNet50 weights), Matplotlib/Seaborn for visualisation.
...
\textbf{Visual features: cold-start is the killer app.}
...
\textbf{Diminishing returns at high $\lambda$.}
