...
PURPOSE:
    ...
    7. Ablation study (ID-only Hero)
    8. Pareto λ-discovery sweep
    9. Pareto front visualisation
   10. Presentation figure generation
...
def parse_args():
    ...
    parser.add_argument("--stage", type=str, default="all",
                        choices=["all", "sample", "embed", "train_villain", "train_hero", "evaluate", "ablation", "pareto_sweep", "pareto_plot", "presentation"],
...
def main():
    ...
    if args.stage in ("all", "sample") and not args.skip_sampling:
        logger.info("[1/10] Sampling data...")
    ...
    if args.stage in ("all", "train_hero"):
        logger.info("[4/10] Training the Hero (main model)...")
        from src.hero.trainer import train_hero
        train_hero(config)

    # Stage 5: Evaluate Both Models
    if args.stage in ("all", "evaluate"):
        logger.info("[5/10] Evaluating both models...")
    ...
    if args.stage in ("all", "evaluate"):
        logger.info("[6/10] Running cold-start analysis...")
    ...
    if args.stage in ("all", "pareto_plot"):
        logger.info("[9/10] Generating Pareto front visualisation...")
        from src.utils.pareto_plot import generate_pareto_plots
        generate_pareto_plots(config)

    # Stage 10: Presentation Figures (Phase 4)
    if args.stage in ("all", "presentation"):
        logger.info("[10/10] Generating presentation figures...")
        from src.phase4_presentation import main as gen_presentation
        gen_presentation()
